...
using static BlueHeron.Collections.Trie.Trie2;

namespace BlueHeron.Collections.Trie.Serialization;

/// <summary>
/// A <see cref="JsonConverter"/> that serializes and deserializes a <see cref="Trie2"/>.
/// </summary>
internal sealed class TrieConverter2 : JsonConverter<Trie2>
{
    #region Fields
    ...
    /// <inheritdoc/>
    public override Trie2? Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        var trie = new Trie2();
    ...
    private static void AddChildren(ref Trie2.Node parent, ref List<NodeWrapper> nodes, ref int curIndex, int numChildren)
    {
        parent.Children = new Trie2.Node[numChildren];
    ...
    /// <inheritdoc/>
    public override void Write(Utf8JsonWriter writer, Trie2 value, JsonSerializerOptions options)
    {
